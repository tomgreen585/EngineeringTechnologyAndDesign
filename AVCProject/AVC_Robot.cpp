...
void turnLeft(){
    ...
    updateSim();
    set_motors(-15,17);
    //setAndUpdateRobot(-5, 40);
    updateSim();
     //added
    //setAndUpdateRobot(-5, 5);//added
   }
...
void initialise(){
    ...
    logic = 1;
    dirs.push('R'); dirs.push('L'); dirs.push('R'); dirs.push('R'); dirs.push('R');
    goStraight(10);
}
...
void runSim(){
    ...
    switch (logic){
        ...
        case 3: if(!enteredChal){goStraight(40); enteredChal=true;}
				runChallenge(); break;
...
int main(){        
    initialise();
    for ( int time = 0 ; time < 5000; time++){ // change upper limit or replace with while(1)to run forever 
        printf("\ntime: %d  ", time);
        runSim();
    }
    return 0;
